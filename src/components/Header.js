...
const Header = () => {
  ...
  return (
    <div className='bg-black text-white flex justify-between items-baseline px-4 py-2'>
      ...
      <button onClick={() => setShow(!show)} className='hidden sm:flex'> 
      {show ? <i className="fa-solid fa-xmark fa-lg"></i> 
      : <i className="fa-solid fa-bars fa-lg"></i>}</button>
      

      <nav className='space-x-5 sm:hidden'>

        {navMenu.map((nav, i) => {
          return <NavLink key={i} to={nav.path} className={' group hover:gray-500 hover:bg-gray-600 active:bg-gray-700 focus:outline-none focus:ring-red-300 ... group-hover:text-black'}>{nav.name}</NavLink>
        })}
